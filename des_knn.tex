...
Figure~\ref{fig:recall-time} (top) shows the primary speed-accuracy trade-off curves, while Table~\ref{tab:results} summarizes key metrics at fixed recall targets (0.95 and 0.99). We report the average query time, the \textit{speedup} factor relative to brute force (based on distance computations), and the \textit{scan ratio} (fraction of dataset scanned) for DES-kNN. For example, at 99\% recall on the clustered synthetic data, DES-kNN (Heuristic) scans only 10\% of the points on average (0.10 scan ratio, $10\times$ speedup), whereas on the uniform data it must scan $\sim$90\% (only $1.1\times$ speedup). HNSW and Annoy do not have a notion of scan ratio since they use indices (we omit those entries); we therefore compare them using matched-recall query time, and comprehensive sweeps with tuned baselines are ongoing.

\begin{figure}[tb]
\centering
% TODO: Insert plots for Recall@10 vs Query Time for each dataset (perhaps as subplots or separate lines on one plot).
% Placeholder description:
%\includegraphics[width=0.48\textwidth]{placeholder_recall_time.png}
\caption{Query performance of DES-kNN vs. baseline methods on four datasets. Each curve shows how recall@10 drops as query time decreases (by adjusting algorithm parameters). DES-kNN is shown in both guarantee mode (dashed line) and heuristic mode (solid line). On structured data (MNIST, Clustered), DES-kNN achieves high recall with substantially less computation than brute force, and can approach the efficiency of index methods. On unstructured data (Uniform), DES-kNN provides little gain.}
\label{fig:recall-time}
\end{figure}
...
\begin{figure}[tb]
\centering
% TODO: Placeholder for a figure with two histograms or boxplots comparing scan_ratio and expected_misses for Clustered vs Uniform.
%\includegraphics[width=0.48\textwidth]{placeholder_metrics.png}
\caption{Distribution of DES-kNN internal metrics (heuristic mode, $\tau=1.0, c=0.99$) for 100 queries on a clustered dataset vs. a uniform dataset. \textbf{Left:} Scan ratio (fraction of dataset scanned) per query. \textbf{Right:} Expected misses at stop. Clustered data allows very low scan ratios and expected misses, indicating effective early stopping, whereas uniform data shows values close to 1.0 (almost full scan).}
\label{fig:metrics}
\end{figure}
